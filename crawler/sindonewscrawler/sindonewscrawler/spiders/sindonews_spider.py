import scrapy
import re
from sindonewscrawler.htmlparser import strip_tags
from sindonewscrawler.items import SindonewsItem

class SindonewsSpider(scrapy.Spider):
	name = "sindonews"
	allowed_domains = ["sindonews.com"]
	start_urls = [
		"http://index.sindonews.com"
	]

	def parse(self, response):
		for i in response.xpath('//*[@class="indeks-news"]//*[@class="indeks-inner"]'):
			item = SindonewsItem()
			#Media Label
			item['media'] = 'Sindo'
			#Date
			...
			#Title
			item['title'] = i.xpath('div[@class="indeks-title"]/a').extract()[0]
			item['title'] = strip_tags(item['title'])
			#Link
			item['link'] = i.xpath('div[@class="indeks-title"]/a/@href').extract()[0]
			#Sinopsis
			item['preview'] = i.xpath('div[@class="indeks-caption"]/span/text()').extract()[0]
	...
	def parse_content(self, response):
		...
		#Date Format
		item['date'] = response.xpath('//div[@class="article"]/time/text()').extract()[0]
		item['date'] = re.sub('^.*,\\xa0 ', '', item['date'])
		item['date'] = re.sub('\\xa0\S*\\xa0 ', '', item['date'])
		...
		#Content
		text_cleaner = response.xpath('//*[@id="content"]').extract()[0]
		...
		#Editor
		if (response.xpath('//span[@class="reporter"]/text()')) :
			item['editor'] = response.xpath('//span[@class="reporter"]/text()').extract()[0]
		else :
			item['editor'] = "Multi Page Detected!!!"

		#Copyright
		if (response.xpath('//a[@rel="author"]/text()')) :
			item['author'] = response.xpath('//a[@rel="author"]/text()').extract()[0]
		elif (response.xpath('//a[@rel="author"]/span/text()')) :
			item['author'] = response.xpath('//a[@rel="author"]/span/text()').extract()[0]
