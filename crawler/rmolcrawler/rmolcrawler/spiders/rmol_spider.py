import scrapy
import re
from rmolcrawler.htmlparser import strip_tags
from rmolcrawler.items import RmolItem

class RmolSpider(scrapy.Spider):
	name = "rmol"
	allowed_domains = ["rmol.co"]
	start_urls = [
		"http://www.rmol.co/indeksberita.php"
	]

	def parse(self, response):
		for i in response.xpath('//div[@class="post-item"]'):
			item = RmolItem()
			#Media Label
			item['media'] = 'RMOL'
			#Date
			item['date'] = i.xpath('div[@class="title"]/p/span/text()').extract()[0]
			item['date'] = strip_tags(item['date'])
			...
			#Title
			item['title'] = i.xpath('div[@class="title"]/h2/a/text()').extract()[0]
			item['title'] = strip_tags(item['title'])
			item['title'] = re.sub('^\s+', '', item['title'])
			item['title'] = re.sub('\s+$', '', item['title'])
			#Link
			item['link'] = i.xpath('div[@class="title"]/h2/a/@href').extract()[0]
			#Sinopsis
			item['preview'] = i.xpath('div[@class="intro hidden-xs"]/text()').extract()[0]
	...
	def parse_content(self, response):
		...
		#Content
		#text_cleaner = response.xpath('//*[@id="primary-column"]/p[2]').extract()[0]
		#text_cleaner = strip_tags(text_cleaner)
		text_cleaner = strip_tags(response.xpath('//div[@class="post newsContent"]').extract()[0])
		if(response.xpath('//div[@class="newsAds"]')):
			text_ads = strip_tags(response.xpath('//div[@class="newsAds"]').extract()[0])
			text_cleaner = re.sub(text_ads, '', text_cleaner)
		text_cleaner = re.sub('[\t\n\r\f\v]', '', text_cleaner)
		...
		#Editor
		try :
			item['editor'] = re.search('\[(.+?)\]$', text_cleaner).group(1)
		except :
			pass
		#Author
		try :
			item['author'] = response.xpath('//div[@class="newsDate"]/p/text()[4]').extract()[0]
