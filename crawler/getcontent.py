...
calc_yesterday = date.today() - timedelta(5)
...
cursor.execute("SELECT id,content,quote1 FROM media_data WHERE date BETWEEN %s AND %s", (yesterday,today))
...
for row in data :
	...
	if(row[2]):
		temp = re.findall(quote_formula,str(row[1]))
		if len(temp) >= 5:
			quote1 = str(temp[0])
			quote2 = str(temp[1])
			quote3 = str(temp[2])
			...
			quote1 = "No Quote"
		print id
		cursor.execute("""UPDATE media_data SET quote1 = %s, quote2 = %s, quote3 = %s, quote4 = %s, quote5 = %s WHERE id = %s""",
