import scrapy
import re
from beritasatucrawler.htmlparser import strip_tags
from beritasatucrawler.items import BeritasatuItem

class BeritasatuSpider(scrapy.Spider):
	name = "beritasatu"
	allowed_domains = ["beritasatu.com"]
	start_urls = [
		"http://www.beritasatu.com/newsindex"
	]

	def parse(self, response):
		for i in response.xpath('//*[@id="content"]/div[3]/div[2]/div[@class="mt10"]'):
			item = BeritasatuItem()
			#Media Label
			item['media'] = 'Berita Satu'
			#Date
			item['date'] = ''
			#Author
			...
			#Title
			item['title'] = i.xpath('div[2]/div[1]/b/a').extract()[0]
			item['title'] = strip_tags(item['title'])
			#Link
			item['link'] = i.xpath('div[2]/div[1]/b/a/@href').extract()[0]
			#Sinopsis
			item['preview'] = i.xpath('div[2]/div[2]/p/text()').extract()[0]
	...
	def parse_content(self, response):
		...
		#Date Format
		item['date'] = response.xpath('//div[@class="content-left"]/div[1]/div/div[1]/span/text()').extract()[0]
		...
		#Content + Copyright
		text_cleaner = response.xpath('//div[@class="f14 c6 bodyp"]').extract()[0]
		...
		text_cleaner = re.sub('\s\s\s\s\s+', ' ||', text_cleaner)
		#Editor
		try :
		...
		#Author
		try :
			item['author'] = re.search(' \|\|(.+?)/', text_cleaner).group(1)
		except :
			pass
		#Source
		try :
			item['source'] = re.search(' \|\|.+?\|\|(.+?)$', text_cleaner).group(1)
		except :
			pass
		#Content Only
