...
import re
import time
from pojoksatucrawler.htmlparser import strip_tags
from pojoksatucrawler.items import PojokSatuItem

today = time.strftime("%Y/%m/%d")


class PojokSatuSpider(scrapy.Spider):
	global today
	name = "pojoksatu"
	allowed_domains = ["pojoksatu.id"]
	start_urls = [
		"http://pojoksatu.id/" + today + "/"
	]

	def parse(self, response):
		for i in response.xpath('//*[@id="index"]/div/div[2]/article//div[@class="post-data"]'):
			item = PojokSatuItem()
			#Media Label
			item['media'] = 'Pojok Satu'
			#Date
			item['date'] = ''
			#Author
			...
			#Title
			item['title'] = i.xpath('div/header/h2/a/text()').extract()[0]
			item['title'] = strip_tags(item['title'])
			#Link
			item['link'] = i.xpath('div/header/h2/a/@href').extract()[0]
			#Sinopsis
	...
	def parse_content(self, response):
		...
		#Date
		date_cleaner = response.xpath('//span[@itemprop="datePublished"]/text()').extract()[0]
		...
		#Content
		pc = 0
		text_sum = response.xpath('//div[@class="post-single-content box mark-links entry-content"]/p').extract()[pc]
		text_check = response.xpath('//div[@class="post-single-content box mark-links entry-content"]/p').extract()[pc]
		for p in response.xpath('//div[@class="post-single-content box mark-links entry-content"]/p') :
		...
		#Author
		try :
			item['author'] = response.xpath('//span[@itemprop="author"]/span[@itemprop="name"]/a/text()').extract()[0]
		except :
			pass
